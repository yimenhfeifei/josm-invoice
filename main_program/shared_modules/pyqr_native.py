...
class QRCode:
    ...
    def makeImage(self):
        ...
        im = QtGui.QImage(pixelsize, pixelsize, Qt.QImage.Format_RGB32)
        im.fill(Qt.qRgb(255, 255, 255))
        painter = QtGui.QPainter(im)
        painter.setPen(Qt.QColor(Qt.QColor.black))
        ...
        for r in range(self.getModuleCount()):
            for c in range(self.getModuleCount()):
                if (self.isDark(r, c) ):
                    x = (c + offset) * boxsize
                    y = (r + offset) * boxsize
                    painter.fillRect(x, y, boxsize, boxsize, Qt.QColor(0, 0, 0))
        return im
    ...
    def createData(typeNumber, errorCorrectLevel, dataList):
        ...
        if (buffer.getLengthInBits() > totalDataCount * 8):
            raise Exception("Code length overflow: {} > {}".format(buffer.getLengthInBits(),
                                                              totalDataCount * 8))
